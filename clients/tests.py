...
class AcountTestCase(APITestCase):
    def test_clients_post(self):
        ...
        response = self.client.post('/clients', data=data, format='json')
        ...
        response = self.client.patch('/clients/1', update_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.put('/clients/1', put_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        put_error_data = {
            "name" : "test_name_02",
        }
        response = self.client.put('/clients/1', put_error_data, format='json')
